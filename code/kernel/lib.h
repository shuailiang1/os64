...
inline int strcmp(char * FirstPart, char * SecondPart) __attribute__((always_inline));
inline int strcmp(char * FirstPart, char * SecondPart) {
    register int __res;
    __asm__ __volatile__(
        "cld	\n\t"             // 清除方向位，确保 LODSB 和 SCASB 指令正常工作
        "1:	\n\t"
        "lodsb	\n\t"           // 将 DS:SI 指向的字节加载到 AL 寄存器，同时 SI 递增
        "scasb	\n\t"           // 将 ES:DI 指向的字节与 AL 比较，同时 DI 递增
        "jne	2f	\n\t"       // 如果不相等，跳转到标号 2 处
        "testb	%%al,	%%al	\n\t" // 测试 AL 是否为零（字符串结束）
        "jne	1b	\n\t"       // 如果不是字符串结束，跳转到标号 1 处继续比较
        "xorl	%%eax,	%%eax	\n\t" // 如果相等且字符串结束，设置返回值为 0
        "jmp	3f	\n\t"       // 跳转到标号 3 处
        "2:	\n\t"              // 标号 2，不相等的情况
        "movl	$1,	%%eax	\n\t" // 将返回值设置为 1
        ...
        "negl	%%eax	\n\t"    // 否则取反
        "3:	\n\t"              // 标号 3，结束
        :"=a"(__res)             // 输出操作数：__res 对应寄存器 EAX
        :"D"(FirstPart), "S"(SecondPart) // 输入操作数：DI 对应 FirstPart，SI 对应 SecondPart
        :                         // clobber list（影响的寄存器）
    );
    return __res;                // 返回结果
}
...
*/
inline int strlen(char * String) __attribute__((always_inline));
